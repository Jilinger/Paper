...
diss_khz=np.array([0.1,0.5,1,10])
atol=10**-6  #10**-6
rtol=10**-6  #10**-3
...
def average(index):
    ...
    initial[0][0]=1
    results = solve_ivp(lambda t, rho: ME(t, rho,H,gamma), [0,times[-1]],initial.ravel(), t_eval=times, atol=atol, rtol=rtol) 
