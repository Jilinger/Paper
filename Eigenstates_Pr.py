...
speicher_atome=np.zeros(len(densities),int)

s=0
for density,radius in zip(densities,radiusse): 
...
start1=time.time()
gebrauchte_zeit=np.zeros(len(densities))
...
for density,radius in zip(densities,radiusse):  
    ...
    text="Eigenstates,IPR+Ratios: density=" +str(np.round(density,3))+", number_atoms="+str(number_atoms)+", average iterations=" + str(iteration)+", gebrauchte Zeit=" +str(gebrauchte_zeit[z])+", r_b=" +str(r_b)
    np.save(data,text)
